...
async fn test_run_evaluation_streaming_success() {
    ...
    let gateway_url = get_gateway_endpoint("")
        .to_string()
        .trim_end_matches('/')
        .to_string();
    let clickhouse_url = std::env::var("TENSORZERO_CLICKHOUSE_URL")
        .unwrap_or_else(|_| "http://localhost:8123/tensorzero_e2e_tests".to_string());
                ...
                "evaluators": {
                    "happy_bool": {
                        "LLMJudge": {
                            ...
                            "cutoff": null,
                            "system_instructions": "Return true if the response is helpful.",
                            "variants": {
                                "dummy": {
                                    "ChatCompletion": {
...
#[tokio::test(flavor = "multi_thread")]
async fn test_run_evaluation_streaming_invalid_gateway_url() {
    let http_client = Client::new();
    ...
    assert_eq!(
        resp.status(),
        reqwest::StatusCode::BAD_REQUEST,
        "Should return 400 for invalid gateway URL"
    );
